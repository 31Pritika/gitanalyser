...
function App() {
  ...
  const [scoreData] = useState({ score: 0, grade: { label: '-', color: '#fff' } })
  ...
  const handleSearch = async (username) => {
    ...
    try {
      ...
      try {
        const summaryRes = await axios.get(`${import.meta.env.VITE_API_URL}/api/summary/${username}`)
      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth: 920, margin: '0 auto', padding: '80px 32px 120px' }}>
        ...
        {!userData && !loading && !error && (
          <p className="anim-delay2" style={{ textAlign: 'center', color: 'rgba(255,255,255,0.2)', fontSize: 13, marginTop: 16 }}>
            Try searching for <span style={{ color: '#a78bfa' }}>torvalds</span>,{' '}
            <span style={{ color: '#60a5fa' }}>gaearon</span>, or your own username
          </p>
        )}
        ...
        {userData && !loading && (
          <div style={{ marginTop: 48, display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <div className="anim-delay1">
              <ProfileScore user={userData} repos={repos} languages={languages} />
            </div>
            {summary && (
            ...
            <div className="anim-delay2">
              {Object.keys(languages).length > 0 && <LanguageChart languages={languages} />}
            </div>
            <div className="anim-delay3">
              {repos.length > 0 && <RepoList repos={repos} />}
            </div>
            <div className="anim-delay4">
              <ShareCard
                user={userData}
                repos={repos}
                languages={languages}
